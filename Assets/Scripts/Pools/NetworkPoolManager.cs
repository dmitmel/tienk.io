...
namespace Tienkio.Pools {
    public class NetworkPoolManager : NetworkBehaviour, IPoolManager {
        ...
        public void Alloc(int objects) {
            if (isServer) poolSize += objects;
        }

        void Create() {
            PoolObject poolObject = Instantiate(prefab, spawnRoot);
        ...
        public void Free(int objects) {
            if (isServer && objects > 0) poolSize = Math.Max(poolSize - objects, 0);
        }

        public void Clear() {
            if (isServer) poolSize = 0;
        }
        ...
        void Update() {
            if (Input.GetKeyDown(KeyCode.Equals)) poolSize++;
            if (Input.GetKeyDown(KeyCode.Minus)) poolSize--;
